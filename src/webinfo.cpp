...
int sendData(int s, int *error, tUrl *myUrl)
{
    ...
    if (write(s, msg_send.c_str(), msg_send.length()) < 0)
...
int readResponse(int s, tUrl *myUrl, tParams *myParams, int *error)
{
    ...
    while (goOn > 0)
    {
        ...
        if (ch == '\n')
        {
            ...
            { // all the other lines to process
                ...
                { // if no setting was set -> print all the lines imediatally
                    ;
                }
            }
            if (myParams->all > 0)
